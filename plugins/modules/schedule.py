...
- name: Build the same schedule using the rrule plugin
  ctrliq.ascender.schedule:
    ...
    state: present
    unified_job_template: "Demo Job Template"
    rrule: "{{ query('ctrliq.ascender.schedule_rrule', 'week', start_date='2019-12-19 13:05:51') | first }}"
  register: result
...
- name: Build a complex schedule for every day except sunday using the rruleset plugin
  ctrliq.ascender.schedule:
    ...
    state: present
    unified_job_template: "Demo Job Template"
    rrule: "{{ query('ctrliq.ascender.schedule_rruleset', '2022-04-30 10:30:45', rules=rrules, timezone='UTC') | first }}"
  vars:
    rrules:
      - frequency: 'day'
        interval: 1
...
RETURN = '''
id:
    description: The numeric database ID of the schedule.
...
def main():
    ...
    # We need to clear out the organization from the search fields as the search for instance_groups doesn't support it and won't be needed anymore
