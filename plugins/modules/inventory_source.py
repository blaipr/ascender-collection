...
options:
    ...
    source:
      ...
      choices: [ "scm", "ec2", "gce", "azure_rm", "vmware", "satellite6", "openstack", "terraform", "ascender" ]
      type: str
    source_path:
      description:
...
def main():
    ...
    argument_spec = dict(
        ...
        #
        source=dict(choices=["scm", "ec2", "gce", "azure_rm", "vmware", "satellite6", "openstack", "terraform", "ascender"]),
        source_path=dict(),
    ...
    inventory_source_fields = {
        'name': new_name if new_name else (module.get_item_name(inventory_source_object) if inventory_source_object else name),
        'inventory': inventory_object['id'],
    }
    ...
    if source_project is not None:
        ...
        if not source_project_object:
            module.fail_json(msg=f'The specified source project, {source_project}, was not found.')
    ...
    if state == 'present' and not inventory_source_object and not inventory_source_fields.get('source'):
