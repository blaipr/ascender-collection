...
|`instance_groups`|Job Template default|no|str|List of Instance Groups applied as a prompt. Job Template default used if not set. Only allowed if `ask_instance_groups_on_launch` set to true on Job Template Set to `[]` to remove all. Omitting this key leaves existing instance groups unchanged.|
  ...
  pre_tasks:
    - name: Include vars from controller_configs directory
      ansible.builtin.include_vars:
        ...
        extensions: ["yml"]
  roles:
    - {role: ctrliq.ascender.schedules, when: controller_schedules is defined}
```
